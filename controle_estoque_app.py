...
def get_app_user_data_dir():
    """Diretório gravável onde o banco será mantido."""
    ...
    return os.path.join(base, "Avitec", "ControleEstoque")

def get_bundle_dir():
    """Diretório onde o EXE/arquivo .py está."""
...
DB_DIR = os.path.join(USER_DATA_DIR, "dados")
DB_PATH = resource_path("dados/estoque.db")
...
def ensure_db():
    ...
    embedded_db = os.path.join(BUNDLE_DIR, "dados", "estoque.db")
    ...
    if not os.path.exists(DB_PATH) and os.path.exists(embedded_db):
        try:
            ...
            print("Aviso: não foi possível copiar DB embutido:", e)
...
def add_produto(nome, categoria_id, preco, quantidade, min_estoque):
    run_query("""
        ...
        VALUES (?, ?, ?, ?, ?)
    """, (nome, categoria_id, preco, quantidade, min_estoque))

def update_produto(pid, nome, categoria_id, preco, quantidade, min_estoque):
    run_query("""
        UPDATE produtos SET nome=?, categoria_id=?, preco=?, quantidade=?, min_estoque=?
        WHERE id=?
    """, (nome, categoria_id, preco, quantidade, min_estoque, pid))
...
def get_produto(pid):
    res = run_query("""
        SELECT id, nome, categoria_id, preco, quantidade, min_estoque
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("Controle de Estoque - AVITEC")
    ...
    def excluir_produto(self):
        ...
        if not messagebox.askyesno("Excluir Produto", f"Excluir produto ID {pid}?"):
            return
    ...
    def create_widgets(self):
        header = Frame(self.root, bg="#edda0e", height=50)
        header.pack(fill=X)

        Label(header, text="AVITEC - Controle de Estoque",
              font=("Arial", 20), bg="#edda0e").pack()
        ...
        Label(self.root, text="Últimas Movimentações:").pack(anchor=W, padx=8)
        self.mov_text = Text(self.root, height=10)
    ...
    def registrar_mov(self, tipo):
        ...
        obs = simpledialog.askstring("Observação", "Observação opcional:")

        add_movimentacao(pid, tipo, qty, obs or "")
        messagebox.showinfo("Registrado", f"{tipo.capitalize()} registrada.")
        self.refresh_produtos()
    ...
    def export_csv(self):
        ...
        with open(path, "w", newline='', encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["ID", "Nome", "Categoria", "Preço", "Quantidade", "MinEstoque"])
            for r in rows:
                w.writerow(r)
    ...
    def on_search(self):
        self.refresh_produtos(self.search_var.get().strip())
    ...
    def refresh_produtos(self, search=None):
        ...
        rows = list_produtos(search)
        for r in rows:
            self.tree.insert("", "end", values=r)

        self.mov_text.delete("1.0", END)
        for m in list_movimentacoes(20):
    ...
    def check_alerts(self):
        ...
        self.root.after(10800000, self.check_alerts)  # 3 horas
...
class ProdutoDialog:
    def __init__(self, parent, produto=None):
        ...
        Label(top, text="Nome:").grid(row=0, column=0)
        self.nome = Entry(top, width=40)
        self.nome.grid(row=0, column=1)

        Label(top, text="Categoria:").grid(row=1, column=0)
        self.cat_cb = ttk.Combobox(top, values=[c[1] for c in list_categorias()])
        self.cat_cb.grid(row=1, column=1)

        Label(top, text="Preço:").grid(row=2, column=0)
        self.preco = Entry(top); self.preco.grid(row=2, column=1)

        Label(top, text="Quantidade:").grid(row=3, column=0)
        self.qtd = Entry(top); self.qtd.grid(row=3, column=1)

        Label(top, text="Estoque Mínimo:").grid(row=4, column=0)
        self.minq = Entry(top); self.minq.grid(row=4, column=1)

        Button(top, text="Salvar", command=self.on_save).grid(row=5, column=0, columnspan=2, pady=5)
